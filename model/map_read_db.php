...
try {
    ...
    if (isset($_POST['user_id'])) {
        $stmt = $db->prepare("SELECT insects.insect_id AS insects_insect_id, insects.insect_name, insects.pref_id, registers.id, registers.register_id,prefs.pref_id, registers.ext FROM insects INNER JOIN prefs ON insects.pref_id = prefs.pref_id INNER JOIN insect_categories ON insects.insect_category_id = insect_categories.category_id LEFT JOIN registers ON insects.insect_id = registers.insect_id AND registers.user_id = :user_id GROUP BY insects.insect_id ORDER BY insects.insect_id ASC;");
